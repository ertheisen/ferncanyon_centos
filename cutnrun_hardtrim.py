...
print 'Current working directory is:' +  os.getcwd()
print '\n'
os.mkdir('hardtrim_out')
os.mkdir('hardtrim_out/fastqc_post_hardtrim')
os.chdir('/data/fastq')
print 'Current working directory is:' +  os.getcwd()
print '\n'
...
print 'Decompressed R1 files to be analyzed:'
print '\n'.join(input_files_R1)
print '\n'

for i in range(len(input_files_R1)):
#create string for system command
	temp_str = 'trim_galore_dev -o /data/hardtrim_out --hardtrim5 30 ' + input_files_R1[i]
...
print 'Decompressed R2 files to be analyzed:'
print '\n'.join(input_files_R2)
print '\n'

for i in range(len(input_files_R2)):
#create string for system command
	temp_str = 'trim_galore_dev -o /data/hardtrim_out --hardtrim5 30 ' + input_files_R2[i]
...
#create string for system command
	temp_str = 'fastqc --outdir=fastqc_post_hardtrim ' + fastqc_input[i]
...
print 'Runtime hard-trimming (hh:mm:ss): ' + str(datetime.now() - startTime)
...
#Determine if fastq files are present
input_fastq = sorted(glob.glob('*.fq'))
input_fastq_R1 = sorted(glob.glob('*R1*.fq'))
input_fastq_R2 = sorted(glob.glob('*R2*.fq'))
...
print 'Runtime quality trimming (hh:mm:ss): ' + str(datetime.now() - startTime)
########################################################
startTime = datetime.now()
...
if len(input_files) == 0:
	print 'No quality-controlled input files from trim_galore'
	print '\n'
